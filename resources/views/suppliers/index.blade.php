...
@section('content')
    <div x-data="supplierPage()" x-cloak>
        ...
        <div class="row g-3 mb-4">
            @foreach([['key' => 'all', 'label' => 'ทั้งหมด', 'tone' => 'primary', 'icon' => 'bi-truck'], ['key' => 'active', 'label' => 'กำลังใช้งาน', 'tone' => 'success', 'icon' => 'bi-check-circle'], ['key' => 'inactive', 'label' => 'ไม่ใช้งาน / พักไว้', 'tone' => 'secondary', 'icon' => 'bi-pause-circle']] as $summary)
                <div class="col-md-4"><a href="{{ route('suppliers.index', array_filter(['q' => $q, 'status' => $summary['key'] === 'all' ? null : $summary['key']])) }}" class="status-summary {{ $status === $summary['key'] ? 'is-selected' : '' }}"><span class="status-summary-icon text-{{ $summary['tone'] }}"><i class="bi {{ $summary['icon'] }}"></i></span><span><small>{{ $summary['label'] }}</small><strong>{{ number_format($counts[$summary['key']]) }}</strong></span></a></div>
            @endforeach
        </div>
...
@push('head')
<style>
    .status-summary { display:flex; align-items:center; gap:12px; height:78px; padding:14px 18px; background:#fff; border:1px solid var(--erp-border); border-radius:14px; color:inherit; text-decoration:none; transition:.15s ease; }
