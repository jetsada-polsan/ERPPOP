...
<main class="pc-shell">
    ...
    <section class="pc-panel pc-section">
        ...
        <div class="pc-table-wrap"><table class="pc-table"><thead><tr><th>เวลา</th><th>สาขา</th><th>ใบเสร็จ</th><th>ช่องทาง</th><th>เลขท้ายบัญชีผู้โอน</th><th>เลขอ้างอิง</th><th class="text-end">ยอด</th></tr></thead><tbody>@forelse($unmatchedTransfers as $payment)<tr><td>{{ \Carbon\Carbon::parse($payment->receipt_date)->format('d/m/Y H:i:s') }}</td><td>{{ $payment->branch_code }} · {{ $payment->branch_name }}</td><td><strong>{{ $payment->receipt_no }}</strong></td><td>{{ ['transfer'=>'โอน','qr'=>'QR','bank'=>'ธนาคาร'][$payment->method] ?? $payment->method }}</td><td>{{ $payment->transfer_account_last4 ?: '-' }}</td><td>{{ $payment->payment_reference ?: '-' }}</td><td class="pc-amount">{{ number_format((float)$payment->amount,2) }}</td></tr>@empty<tr><td colspan="7" class="text-center text-success py-4"><i class="bi bi-check-circle me-1"></i>ไม่มีรายการ QR/โอนค้างตรวจ</td></tr>@endforelse</tbody></table></div>
    </section>
</main>
@endsection
