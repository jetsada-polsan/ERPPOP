...
@push('head')
<style>
    [x-cloak]{display:none!important}.ma-shell{display:grid;gap:14px}.ma-panel{background:#fff;border:1px solid var(--erp-border);border-radius:8px}.ma-filter{display:flex;gap:10px;align-items:end;padding:13px;flex-wrap:wrap}.ma-field label{display:block;margin-bottom:4px;color:#667f90;font-size:11px;font-weight:800}.ma-field input,.ma-field select,.ma-field textarea{border:1px solid var(--erp-border);border-radius:6px;color:var(--erp-primary-dark);font-size:12px}.ma-field input,.ma-field select{height:38px;padding:0 9px}.ma-field textarea{width:100%;min-height:70px;padding:9px}.ma-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr))}.ma-stat{padding:14px 16px;border-right:1px solid #e6edf2}.ma-stat:last-child{border:0}.ma-stat span{display:block;color:#718696;font-size:10px;font-weight:900}.ma-stat strong{color:var(--erp-text);font-size:20px}.ma-tabs{display:flex;gap:5px;padding:8px;border-bottom:1px solid var(--erp-border);overflow-x:auto}.ma-tab{height:35px;padding:0 13px;border:0;border-radius:6px;background:transparent;color:#61798a;font-size:12px;font-weight:900;white-space:nowrap}.ma-tab.active{color:#fff;background:var(--erp-primary-dark)}.ma-section{padding:15px}.ma-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}.ma-head h2{margin:0;color:var(--erp-primary-dark);font-size:16px;font-weight:900}.expense-form{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.span-2{grid-column:span 2}.span-4{grid-column:span 4}.ma-table-wrap{overflow-x:auto}.ma-table{width:100%;min-width:980px;border-collapse:collapse}.ma-table th{padding:9px 10px;background:#f6f9fb;color:#61798a;border-bottom:1px solid var(--erp-border);font-size:10px;font-weight:900}.ma-table td{padding:9px 10px;color:#425f73;border-bottom:1px solid var(--erp-surface-2);font-size:11px;vertical-align:middle}.ma-table tr:last-child td{border:0}.status-pill{display:inline-flex;padding:4px 7px;border-radius:5px;font-size:10px;font-weight:900}.status-matched{color:var(--erp-success-ink);background:var(--erp-success-soft)}.status-mismatch{color:var(--erp-danger);background:var(--erp-danger-soft)}.status-pending{color:#8a5a00;background:var(--erp-warning-soft)}.reconcile-form{display:grid;grid-template-columns:130px 130px 130px 1fr auto;gap:7px;align-items:end;padding:10px;background:#f8fafb;border-radius:6px}.export-band{display:grid;grid-template-columns:1fr auto;gap:15px;align-items:center;padding:16px;border-left:4px solid var(--erp-primary)}.export-band h3{margin:0 0 4px;color:var(--erp-primary-dark);font-size:15px;font-weight:900}.export-band p{margin:0;color:#6b8190;font-size:11px}.ma-note{padding:11px 13px;color:#6b8190;background:#f6fafc;border:1px solid var(--erp-border);border-radius:6px;font-size:11px;line-height:1.5}@media(max-width:900px){.ma-stats{grid-template-columns:repeat(2,1fr)}.ma-stat:nth-child(2){border-right:0}.expense-form{grid-template-columns:repeat(2,1fr)}.span-4{grid-column:span 2}.reconcile-form{grid-template-columns:1fr 1fr}.reconcile-form .wide{grid-column:span 2}}@media(max-width:560px){.ma-stats,.expense-form{grid-template-columns:1fr}.ma-stat{border-right:0;border-bottom:1px solid #e6edf2}.span-2,.span-4{grid-column:span 1}.export-band{grid-template-columns:1fr}.reconcile-form{grid-template-columns:1fr}.reconcile-form .wide{grid-column:span 1}}
</style>
@endpush
...
<div class="ma-shell" x-data="{ tab: 'expenses', reconcileId: null, accountModalOpen: false, accountEditingId: null, accountCode: '', accountNameTh: '', accountNameEn: '', accountFormAction: '{{ route('chart-of-accounts.store') }}', openAccountCreate() { this.accountEditingId = null; this.accountCode = ''; this.accountNameTh = ''; this.accountNameEn = ''; this.accountFormAction = '{{ route('chart-of-accounts.store') }}'; this.accountModalOpen = true; }, openAccountEdit(id, code, nameTh, nameEn) { this.accountEditingId = id; this.accountCode = code; this.accountNameTh = nameTh; this.accountNameEn = nameEn || ''; this.accountFormAction = '{{ url('/chart-of-accounts') }}/' + id; this.accountModalOpen = true; } }" x-cloak>
    ...
    <section class="ma-panel ma-section">
        ...
        <div class="row g-2 mb-3">
            ...
            <div class="col-md-3"><div class="ma-note" style="border-left:4px solid {{ $posControl['transfer_unmatched_count'] ? '#dc3545':'#198754' }}"><strong>รายการต้องตรวจ</strong><br><span style="color:{{ $posControl['transfer_unmatched_count'] ? '#b42318':'var(--erp-success-ink)' }}">{{ number_format($posControl['transfer_unmatched_count']) }} ใบ · ฿{{ number_format($posControl['transfer_unmatched_amount'],2) }}</span><br><span class="text-muted">เปิดแท็บ Statement เพื่อตรวจ/แนบสลิป</span></div></div>
        </div>
