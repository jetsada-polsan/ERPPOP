...
- จำนวนตารางที่แอปสร้างและใช้งาน: **6 ตาราง**
- จำนวนคอลัมน์รวม: **34 คอลัมน์**
...
ภาพ ERD นี้แสดงตารางแกนหลักเดิม ส่วน `offline_cashiers` และ `pos_sale_history` อธิบายไว้ด้านล่าง
...
### 4. `offline_cashiers` — แคชพนักงานสำหรับล็อกอินเมื่อออฟไลน์

| คอลัมน์ | ชนิด | เก็บข้อมูลอะไร |
|---|---|---|
| `id` | `INTEGER` | รหัสพนักงานเดียวกับ ERP |
| `code` | `TEXT` | รหัสพนักงาน เช่น `POP001` |
| `name` | `TEXT` | ชื่อพนักงานที่แสดงบน POS |
| `branch_id` | `INTEGER` | สาขาที่พนักงานมีสิทธิ์ใช้งาน |
| `credential` | `TEXT` | JSON salt/verifier สำหรับตรวจ PIN แบบออฟไลน์ ไม่ใช่ PIN ตัวจริง |
| `active` | `INTEGER` | สถานะใช้งาน 1 หรือ 0 |
| `synced_at` | `TEXT` | วันเวลาที่ซิงก์ข้อมูลพนักงานล่าสุด |

PIN แบบออฟไลน์จะถูกเก็บหลังพนักงานล็อกอินออนไลน์สำเร็จเท่านั้น และมีวันหมดอายุตามที่ ERP กำหนด

### 5. `checkout_queue` — คิวบิลขายที่รอส่ง ERP
...
### 6. `pos_sale_history` — ประวัติใบเสร็จย้อนหลังในเครื่อง POS
...
offline_cashiers
  └── id       -> อ้างถึง cashier_id ใน checkout_queue.payload
